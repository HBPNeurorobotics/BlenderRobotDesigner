...
def export_rqtez_publisher(topic_list, path_to_output_file, min_value, max_value, publish_interval_value, is_repeat_value):
    '''
    ...
    with open(path_to_output_file, 'w+') as output_file:
        ...
        for topic_name in topic_list:
            string = '  ' + topic_name + ': {is_repeat: ' + is_repeat_value \
...
def export_rqtez_publisher_muscle(toplevel_directory):
    ...
    path_to_output_file = os.path.join(toplevel_directory, 'rqt/rqt_ez_pub_muscles.yaml')
    model_name = global_properties.model_name.get(bpy.context.scene)

    muscles = [obj.name for obj in bpy.data.objects if
               obj.RobotDesigner.muscles.robotName == global_properties.model_name.get(bpy.context.scene)]

    topic_list = ['/gazebo_muscle_interface/' + model_name + '/' + m + '/cmd_activation/data' for m in muscles]
    ...
    # Write yaml file
    export_rqtez_publisher(topic_list=topic_list, path_to_output_file=path_to_output_file, min_value=min_value, \
                           max_value=max_value, publish_interval_value=publish_interval_value, is_repeat_value=is_repeat_value)
...
def export_rqtez_publisher_controller(toplevel_directory):
    ...
    publish_interval_value = 100
    is_repeat_value = 'false'
    path_to_output_file = os.path.join(toplevel_directory, 'rqt/rqt_ez_pub_jointcontroller.yaml')

    model_name = global_properties.model_name.get(bpy.context.scene)
    controller_links = [obj for obj in bpy.data.objects[model_name].data.bones if
                obj.RobotDesigner.jointController.isActive]

    topic_list_vel = ['/' + model_name + '/' + j.RobotDesigner.joint_name + '/cmd_vel/data' for j in controller_links if
                      j.RobotDesigner.jointController.controllerType == 'velocity']
    topic_list_pos = ['/' + model_name + '/' + j.RobotDesigner.joint_name  + '/cmd_pos/data' for j in controller_links if
                      j.RobotDesigner.jointController.controllerType == 'position']
...
class RQT_Multiplot_Diagram:
    ...
    def __init__(self, title, x_axis_description, y_axis_description, curve_titles, rostopics, fields, types):
        self.title_ = title
...
def export_rqt_multiplot(diagram_array, path_to_output_file, num_of_rows, num_of_columns, render_antialias):
    '''
    ...
    with open(path_to_output_file, 'w+') as output_file:
        ...
        for row_id in range(num_of_rows):
            ...
            for column_id in range(num_of_columns):
                ...
                string = '\t\t\t\t<column_' + str(column_id) + '>\n' \
                         + '\t\t\t\t\t<axes>\n' \
                         + '\t\t\t\t\t\t<axes>\n' \
                         + '\t\t\t\t\t\t\t<x_axis>\n' \
                         + '\t\t\t\t\t\t\t\t<custom_title>' + diagram_array[
                             diagram_id].x_axis_description_ + '</custom_title>\n' \
                         + '\t\t\t\t\t\t\t\t<title_type>0</title_type>\n' \
                         + '\t\t\t\t\t\t\t\t<title_visible>true</title_visible>\n' \
                         + '\t\t\t\t\t\t\t</x_axis>\n' \
                         + '\t\t\t\t\t\t\t<y_axis>\n' \
                         + '\t\t\t\t\t\t\t\t<custom_title>' + diagram_array[
                             diagram_id].y_axis_description_ + '</custom_title>\n' \
                         + '\t\t\t\t\t\t\t\t<title_type>0</title_type>\n' \
                         + '\t\t\t\t\t\t\t\t<title_visible>true</title_visible>\n' \
                         + '\t\t\t\t\t\t\t</y_axis>\n' \
                         + '\t\t\t\t\t\t</axes>\n' \
                ...
                for curve_id in range(diagram_array[diagram_id].num_of_curves_):
                    string = '\t\t\t\t\t\t<curve_' + str(curve_id) + '>\n' \
                             ...
                             + '\t\t\t\t\t\t\t\t\t\t<type>2</type>\n' \
                             + '\t\t\t\t\t\t\t\t\t</scale>\n' \
                             + '\t\t\t\t\t\t\t\t\t<topic>' + diagram_array[diagram_id].rostopics_[
                                 curve_id] + '</topic>\n' \
                             + '\t\t\t\t\t\t\t\t\t<type>' + diagram_array[diagram_id].types_[curve_id] + '</type>\n' \
                             + '\t\t\t\t\t\t\t\t</x_axis>\n' \
                             + '\t\t\t\t\t\t\t\t<y_axis>\n' \
                             + '\t\t\t\t\t\t\t\t\t<field>' + diagram_array[diagram_id].fields_[curve_id] + '</field>\n' \
                             ...
                             + '\t\t\t\t\t\t\t\t<render_antialias>' + render_antialias + '</render_antialias>\n' \
                             ...
                             + '\t\t\t\t\t\t\t<title>' + diagram_array[diagram_id].curve_titles[curve_id] + '</title>\n' \
                             + '\t\t\t\t\t\t</curve_' + str(curve_id) + '>\n'
                    output_file.write(string)
                ...
                string = '\t\t\t\t\t</curves>\n' \
                         ...
                         + '\t\t\t\t\t<title>' + diagram_array[diagram_id].title_ + '</title>\n' \
                         + '\t\t\t\t</column_' + str(column_id) + '>\n'
                output_file.write(string)
...
def export_rqt_multiplot_muscles(toplevel_directory):
    ...
    # Set parameters
    path_to_output_file = os.path.join(toplevel_directory, 'rqt/rqt_multiplot_muscles.xml')
    model_name = global_properties.model_name.get(bpy.context.scene)

    muscles = [obj.name for obj in bpy.data.objects if
               obj.RobotDesigner.muscles.robotName == global_properties.model_name.get(bpy.context.scene)]

    topic_list = ['/gazebo_muscle_interface/' + model_name + '/' + m + '/cmd_activation' for m in muscles]
    ...
    diagram_array = []
    diagram = RQT_Multiplot_Diagram('Muscle Actuation', 'time', 'actuation', \
    ...
    for sensor in ['length', 'lengthening_speed', 'force']:
        diagram = RQT_Multiplot_Diagram('Muscle Sensors ' + sensor, 'time', sensor, \
                                          muscles, \
                                          muscle_number * ['/gazebo_muscle_interface/' + model_name + '/muscle_states'], \
                                          ['muscles/' + str(i) + '/' + sensor for i in range(0, muscle_number)], \
                                          muscle_number * ['gazebo_ros_muscle_interface/MuscleStates'])
        diagram_array.append(diagram)

    export_rqt_multiplot(diagram_array=diagram_array, path_to_output_file= path_to_output_file, num_of_rows=num_of_rows, \
                         num_of_columns=num_of_columns, render_antialias=render_antialias)
...
def export_rqt_multiplot_jointcontroller(toplevel_directory):
    ...
    # Set parameters
    path_to_output_file = os.path.join(toplevel_directory, 'rqt/rqt_multiplot_jointcontroller.xml')

    model_name = global_properties.model_name.get(bpy.context.scene)
    controller_links = [obj for obj in bpy.data.objects[model_name].data.bones if
                obj.RobotDesigner.jointController.isActive]

    topic_list_vel = ['/' + model_name + '/' + j.RobotDesigner.joint_name + '/cmd_vel' for j in controller_links if
                      j.RobotDesigner.jointController.controllerType == 'velocity']
    topic_list_pos = ['/' + model_name + '/' + j.RobotDesigner.joint_name  + '/cmd_pos' for j in controller_links if
                      j.RobotDesigner.jointController.controllerType == 'position']
    ...
    # Create an array of Diagram instances
    diagram_array = []
    diagram = RQT_Multiplot_Diagram('Joint Actuation', 'time', 'actuation', \
                                    topic_list, \
                                    topic_list, \
                                    joint_number * ['data'], \
    ...
    for sensor in ['position', 'velocity', 'effort']:
        diagram = RQT_Multiplot_Diagram('Joint Sensors ' + sensor, 'time', sensor, \
                                        [topic_list], \
    ...
    export_rqt_multiplot(diagram_array=diagram_array, path_to_output_file=path_to_output_file,
