...
class COLLADA(object):
    ...
    VISUAL_SCENE_TYPE_SID = VISUAL_SCENE_ID+'//{ns}node[@sid="{sid}"][@type="{type}"]' #Type either 'JOINT' or 'NODE'
    ...
    def attach(self, armature, meshes=None):
        ...
        bind_kinematics_model = self.SubElement(instance_kinematics_scene,
                '{ns}bind_kinematics_model' ,{'node': armature.name}) #as soon as we can.
        self.SubElement(bind_kinematics_model,'{ns}param').text = bind.get('symbol')

        def parseArmature(armatureJoint,parent,depth=0):


            joint = self.SubElement(library_joints,'{ns}joint',{'id':armatureJoint.name+'_joint','name':armatureJoint.name+'_joint'})
            ...
            #Axis
            newparam=self.SubElement(instance_kinematics_model,'{ns}newparam',
                    {'sid':self.makeSID(AS_kinematics,instance_kinematics_model,instance_joint,axis_type)})
            ...
            #Initial Values
            newparam=self.SubElement(instance_kinematics_model,'{ns}newparam',
                    {'sid':self.makeSID(AS_kinematics,instance_kinematics_model,instance_joint)+'_value'})
            ...
            axis_info_kinematics = self.SubElement(AS_kinematics_technique,
                    '{ns}axis_info',{'axis': self.makeSIDREF(kinematics_model,instance_joint,axis_type),
            ...
            axis_info_motion = self.SubElement(AS_motion_technique,
                    '{ns}axis_info',{'axis': self.makeSIDREF(AS_kinematics,axis_info_kinematics)})
...
#            el.set('sid','offset')
            ...
            for sensor,position in armatureJoint.markers:
                extra = self.SubElement(AS_motion,'{ns}extra', {'type':'attach_sensor', 'name':sensor}) #.replace('VICON_','')})
                ...
                frame_origin = self.SubElement(technique,'{ns}frame_origin',{'link':self.makeSIDREF(kinematics_model,link)})
        ...
        root_link=self.SubElement(kinematics_model_technique,'{ns}link',{'sid': armature.name+'_root_link','name': armature.name+'_root_link'})
                ...
                for attachment in self.findall('{ns}attachment_full',link,warning=False):
                    ...
                    for axis in joint.findall('*[@sid]'): #TODO We limit ourselves to one axis per joint!
                        ...
                        self.logger.debug('%sactive: %s, locked: %s, min: %s, max: %s', space,
                                axis_info_kinematics[0][0].text, axis_info_kinematics[1][0].text, axis_info_kinematics[2][0][0].text,
                        ...
                        self.logger.debug('%sspeed: %s, acceleration: %s, deceleration: %s, jerk: %s', space,
                                axis_info_motion[0][0].text, axis_info_motion[1][0].text, axis_info_motion[2][0].text,
    ...
    def addMassObject(self,name,transformations, inertia,mass):
        ...
        :type transformations: list of tuples (with 3 or 4 elements each, representing translations and rotations respectively)
        :param inertia: The diagonal elements of the inertia tensor (note that it has to be axis aligned to the local coordinate frame)
...
class Tree(object):
    ...
    def compare(self,other,depth=0):
        def check(v1,v2,depth,marker):
            ...
            print( "%s %s %s %s %s %s" % (str(marker*2*depth), str(v1), str(v2), str(type(v1)), str(type(v1)), str(match)))
...
#        if sid:
#            return self.find('{ns}library_kinematics_models/{ns}kinematics_model[@id="{id}"]/{ns}technique_common//*[@sid="{sid}"]',
#                id=id,sid=sid)
#        else:
